...
const EditAmenity: React.FC<EditAmenityProps> = ({ data, open, onOpenChange, form, validationErrors }) => {
    
    const [imagePreview, setImagePreview] = React.useState<string | null>(data.icon || null);
    React.useEffect(() => {
        if (open) setImagePreview(data.icon || null);
      }, [open]);
    if (!open) return null;

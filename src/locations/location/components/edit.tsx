        ...
        (async () => {
          try {
            ...
            setCountryOptions(items.map((c: any) => ({ value: c.id, label: c.name })));
              if (location.country) {
                try {
                  const cityFilterObj = { filter: [ { search_field: 'country', search_value: location.country } ] } as SearchParams;
                  const cityOpts = getCitiesQuery(1, 100, '', '', cityFilterObj)();
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  const cityRes = cityOpts && typeof cityOpts.queryFn === 'function' ? await (cityOpts.queryFn as any)() : await fetchCities(1, 100, '', '', cityFilterObj);
                  if (!mounted) return;
                  const cityItems = cityRes?.data ?? [];
                  setCityOptions(cityItems.map((c: any) => ({ value: c.id, label: c.name })));
                } catch (err) {
                  // ignore
                }
            ...
            setCityOptions([]);
              (async () => {
                try {
                  ...
                  if (value) {
                    filterObj = { filter: [ { search_field: 'country', search_value: value } ] };
                  }
                  const cityOpts = getCitiesQuery(1, 100, undefined, undefined, filterObj)();
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  const cityRes = cityOpts && typeof cityOpts.queryFn === 'function' ? await (cityOpts.queryFn as any)() : await fetchCities(1, 100, undefined, undefined, filterObj);
                  const cityItems = cityRes?.data ?? [];
                  setCityOptions(cityItems.map((c: any) => ({ value: c.id, label: c.name })));
                } catch (err) {
                  // ignore
                ...
                <Label className="text-sm font-medium">City</Label>
                    <div className="mt-2">
                        <SearchableSelect
                            ...
                            try {
                                let filterObj: SearchParams | undefined;
                                  const countryFilterValue =  form.country;

...
import type { Amenity, CreateAmenityDTO, UpdateAmenityDTO } from "@/types/attribute/index.";
import type { ApiResponse } from "@/types/common";


export const createAmenity = (payload: CreateAmenityDTO) => {
  const { name, icon } = payload;
  return protectedApi.post<ApiResponse<any>>('/attributes/amenity', { name, icon });
}
...
export const updateAmenity = (id: string, payload: UpdateAmenityDTO) => {
  ...
  if (icon && icon.startsWith("data:")) {
    ...
    return protectedApi.patch<ApiResponse<any>>(`/attributes/amenity/${id}`, { name, icon });
  }else {
    // If the icon is a URL, we should send it as is
    return protectedApi.patch<ApiResponse<any>>(`/attributes/amenity/${id}`, { name, icon:null });
  }
...
export const statusChangeAmenity = (id: string, status: boolean) => {
  return protectedApi.patch<ApiResponse<any>>(`/attributes/amenity/${id}/status`, { status });
}